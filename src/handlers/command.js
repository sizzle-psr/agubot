...
async function async_command_handler(
  twitch_client,
  separated_command,
  pg_client,
  channel,
      ...
      if (separated_command[1] === "add") {
        if (res.rows.length === 0) {
          ...
          if (!isAlias) {
            twitch_client.say(
              channel,
              "Command " + separated_command[2] + " already exists."
            );
          } else {
            twitch_client.say(
              channel,
              "Alias " + separated_command[2] + " already exists."
            );
          }
        }
        ...
        if (res.rows.length === 1) {
          ...
          if (!isAlias) {
            twitch_client.say(
              channel,
              "Command " + command_name + " was updated."
            );
          } else {
            twitch_client.say(
              channel,
              "Alias " + command_name + " was updated."
            );
          }
        } else {
          if (!isAlias) {
            twitch_client.say(
              channel,
              "Command " + separated_command[2] + " does not exist."
            );
          } else {
            twitch_client.say(
              channel,
              "Alias " + separated_command[2] + " does not exist."
            );
          }
        }
      } else {
        ...
        if (res.rows.length === 1) {
          ...
          // Upload to db
          database_utils.deleteCommandFromChannel(
            command_name,
            channel,
            pg_client
          );
          ...
          if (!isAlias) {
            ...
            twitch_client.say(
              channel,
              "Alias " + command_name + " was deleted."
            );
          }
        } else {
          if (!isAlias) {
...
function command_handler(
  ...
  if (
    ...
    if (
      ...
      !separated_command[3].startsWith("!") &&
      isAlias
    ) {
