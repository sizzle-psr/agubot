...
async function async_permission_handler(
  separated_command,
  twitch_client,
  channel,
  pg_client
  ...
  await pg_client
    ...
    .then((res) => {
      ...
      if (res.rows.length === 1) {
        ...
        twitch_client.say(
          channel,
          "Permissions updated for " + separated_command[2]
        );
      } else {
        // There is no command with this name in the database
        if (default_commands.indexOf(separated_command[2]) != -1) {
          if (
            non_permission_downgrade.indexOf(separated_command[2]) != -1 &&
            permission < 2
          ) {
            twitch_client.say(
              channel,
              "You can't downgrade permissions below mod for " +
                separated_command[2]
            );
          } else {
            insertDefaultCommandWithPermission(
          ...
          twitch_client.say(
            channel,
            "Command " + separated_command[2] + " is not defined"
          );
        }
      }
    })
...
function permission_handler(
  ...
  if (
    ...
    if (separated_command[1] === "broadcaster" && !is_broadcaster) {
      twitch_client.say(
        channel,
        "Only broadcasters can elevate commands to broadcaster permission"
      );
    } else {
