...
async function async_counter_handler(
  twitch_client,
  separated_command,
  pg_client,
  channel
    ...
    .then((res) => {
      ...
      if (separated_command[1] === "add") {
        if (res.rows.length === 0) {
          ...
          // Upload to db
          database_utils.insertCounterFromChannel(
            counter_name,
            channel,
            pg_client
          );
          ...
          twitch_client.say(
            channel,
            "Counter " + separated_command[2] + " already exists."
          );
        }
        ...
        if (res.rows.length === 1) {
          ...
          if (separated_value.length != 2) {
            twitch_client.say(
              channel,
              "Correct syntax: !counter edit <name> <upper>/<lower>"
            );
            return;
          }
          ...
          twitch_client.say(
            channel,
            "Counter " + counter_name + " was updated."
          );
        } else {
          twitch_client.say(
            channel,
            "Counter " + separated_command[2] + " does not exist."
          );
        }
      } else {
        // Must be delete
        if (res.rows.length === 1) {
          ...
          // Upload to db
          database_utils.deleteCounterFromChannel(
            counter_name,
            channel,
            pg_client
          );
          ...
          twitch_client.say(
            channel,
            "Counter " + counter_name + " was deleted."
          );
        } else {
          twitch_client.say(
            channel,
...
async function async_display_counter(
  separated_command,
  channel,
  twitch_client,
  pg_client,
    ...
    .then((res) => {
      if (res.rows.length === 0) {
        if (is_mod) {
          twitch_client.say(
            channel,
            "Counter " + separated_command[0] + " does not exist."
          );
        }
      } else {
        ...
        if (total === 0)
          percentage =
            "I can do a lot of things, diving by 0 is not one of them";
        twitch_client.say(
          channel,
          separated_command[0] +
            ": " +
...
async function async_update_counter(
  separated_command,
  channel,
  twitch_client,
  pg_client
