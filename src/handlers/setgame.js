...
async function async_setgame_handler(
  game,
  category,
  channel,
  twitch_client,
  ...
  const url_categrories =
    "https://www.speedrun.com/api/v1/games/" + game_abbrv + "/categories";
  ...
  if (var_result_json.data.length === 0) {
    ...
    for (i = 0; i < var_result_json.data.length; i++) {
      ...
      var_value_string =
        var_value_string + " " + var_result_json.data[i].values.default;
    }
    insertGameCategoryFromChannelWithVar(
      pg_client,
      game,
...
function setgame_handler(separated_command, channel, twitch_client, pg_client) {
  ...
  if (new_string.length < 2) {
    twitch_client.say(
      channel,
      "Correct syntax: !setgame <game> | <category> [| <variable>]"
    );
    return;
  } else {
    async_setgame_handler(
      new_string[0].trim(),
      new_string[1].trim(),
      channel,
      twitch_client,
