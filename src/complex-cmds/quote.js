...
function handler(separated) {
    ...
    if (separated.length < 2) {
        if (quote_list.length === 0) {
            return [RetCodes.ERROR, 'Cannot fetch quote of empty list.'];
        }

        let num = Math.floor(Math.random() * quote_list.length) + 1;
        return [RetCodes.OK, 'Quote #' + num + ' by ' + quote_list[num-1].user + 
                ': ' + quote_list[num-1].quote];
    }

    if (separated[1] === 'add') {
        if (separated.length < 4) {
            return [RetCodes.ERROR, 'Correct Syntax: !quote add <user> <quote>'];
        }
        ...
        update_quote_db();
        return [RetCodes.OK, 'Quote #' + (size+1) + ' by ' + quote_list[size].user + ' was added.'];
    }

    if (separated[1] === 'delete') {
        if (separated.length !== 3) {
            return [RetCodes.ERROR, 'Correct Syntax: !quote delete <number>'];
        }

        let num = Number(separated[2]);

        if (!Number.isInteger(num) || num < 1) {
            return [RetCodes.ERROR, 'Please provide a positive integer for the quote number.']
        }

        if (num > quote_list.length) {
            return [RetCodes.ERROR, 'Quote #' + num + ' does not exist.'];
        }
        delete quote_list[num-1];
        quote_list.splice(num-1, 1);
        update_quote_db();
        return [RetCodes.OK, 'Quote #' + num + ' was deleted.'];
    }
    ...
    if (num_quote > quote_list.length) {
        return [RetCodes.ERROR, 'Quote #' + num_quote + ' does not exist.'];
    }

    return [RetCodes.OK, 'Quote #' + num_quote + ' by ' + quote_list[num_quote-1].user + 
