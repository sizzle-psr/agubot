...
async function print_pb(res_runner_db, res, twitch_client, channel_name) {
  ...
  url = 'https://www.speedrun.com/api/v1/users/' + runner_id + '/personal-bests?embed=game,category';
  ...
  if (res.status != 200 || res_json.data.length == 0) {
    twitch_client.say(channel_name, 'No PB found for runner ' + runner_name + '.');
    return;
  }
  ...
  for (i = 0; i < res_json.data.length; i++) {
    ...
    if (run.game.data.abbreviation === game_abbrv && run.category.data.id === cat_id) {
      var good_vars = true;
...
async function async_handler_async_handler(res_runner_db, twitch_client, channel_name, pg_client) {
  const query = {
...
async function async_handler(channel_name, twitch_client, pg_client) {
  ...
  await pg_client.query(query).then((res) => {
    if (res.rows.length === 0) {
      twitch_client.say(channel_name, 'No runner is set. Please use !setrunner to set a runner.');
      return;
