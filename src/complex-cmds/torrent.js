...
function handler(separated) {
    ...
    if (!Number.isInteger(maxhp) || maxhp < 0 || Math.floor(maxhp) !== maxhp) {
        return [RetCodes.ERROR, 'Please provide a positive integer for the max hp.'];
    } else if (maxhp < 3) {
        let alternatives = [
        ...
        return [RetCodes.ERROR, alternatives[Math.floor(Math.random()*alternatives.length)]];
    } else if (maxhp > 714 ) {
        return [RetCodes.ERROR, 'The maximum possible hp for a pokemon is 714 for a Blissey. And even they don\'t have torrent.'];
    } else {
        var message = message = Math.floor(maxhp/3) + ' or less HP.';
        if (maxhp == 69) { // For the memes
