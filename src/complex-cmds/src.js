...
function handler(separated, client, target) {
  ...
  const request = async () => {
    try {
      ...
      var cat_id;
      res = await fetch(url_game, agentOptions);
      ...
      let url_variable =
        "https://www.speedrun.com/api/v1/categories/" + cat_id + "/variables";

      var variables = [];
      res = await fetch(url_variable, agentOptions);
      res_json = await res.json();
      ...
      if (subcategory) {
        ...
        for (var variable in res_json.data) {
          for (var key in res_json.data[variable].values.values) {
            if (
              ...
              res_json.data[variable].values.values[key].label.toLowerCase()
            ) {
        ...
        // Take default value
        for (var variable in res_json.data) {
          variables[res_json.data[variable].id] =
            res_json.data[variable].values.default;
        }
      }
      ...
      var message = "ok";
      var termination;
      res = await fetch(url, agentOptions);
